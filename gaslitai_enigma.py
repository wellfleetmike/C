...
Usage:
    python3 gaslitai_enigma.py <file>
    python3 gaslitai_enigma.py <file> --full
    python3 gaslitai_enigma.py <directory> --scan
...
def get_context(text, pos, window=40):
    ...
    # Strip hidden chars from context for readability
    before_clean = ''
    for c in before:
...
def classify_placement(before, after):
    ...
    semantic_indicators = [
        ...
        any(word in (before + after).lower() for word in
...
def analyze_intent(result):
    ...
    ambiguous_count = sum(1 for f in findings if f['placement'] == 'AMBIGUOUS')
    ...
    if structural_count > semantic_count and not multi_layer:
        return 'LIKELY_ARTIFACT', 'Probably encoding debris: ' + '; '.join(signals) if signals else 'Structural placement suggests format conversion artifacts'
    ...
    return 'INDETERMINATE', 'Mixed signals: ' + '; '.join(signals) if signals else 'Insufficient pattern for classification'
...
def print_shadow_report(result, full=False):
    ...
    print("  Layers detected: %s" % ', '.join(result['layers']) if result['layers'] else 'none')
    print("  Eras: %s" % ', '.join(result['eras']) if result['eras'] else 'none')
    ...
    if signal:
        ...
        for i, f in enumerate(signal):
            ...
            if full:
                ...
                marker = '*' if placement == 'SEMANTIC' else '.' if placement == 'STRUCTURAL' else '?'
                print("  %s ...%s [%s] %s..." % (
                    marker,
                    f['before'][-25:],
                    info['name'],
...
def scan_directory(dir_path):
    ...
    results.sort(key=lambda x: (
        0 if x['intent'] == 'LIKELY_DELIBERATE' else
        1 if x['intent'] == 'SUSPICIOUS' else
        2 if x['intent'] == 'INDETERMINATE' else
        3 if x['intent'] == 'LIKELY_ARTIFACT' else 4,
        -x['hidden']
    ))
...
def main():
    ...
    for arg in args:
        if arg == '--full':
            ...
            print("GaslitAI Enigma v1.0 — Hidden Message Compiler")
            print("")
            print("Usage:")
            print("  python3 gaslitai_enigma.py <file>           Extract shadow message")
            print("  python3 gaslitai_enigma.py <file> --full    Full analysis with layers")
            print("  python3 gaslitai_enigma.py <dir> --scan     Scan and rank all files")
            print("")
    ...
    if not target:
        print("GaslitAI Enigma v1.0")
        print("Usage: python3 gaslitai_enigma.py [--full|--scan] <file_or_directory>")
        sys.exit(1)
